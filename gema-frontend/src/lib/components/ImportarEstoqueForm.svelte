	...
	async function handleSubmit() {
		...
		try {
			...
			if (e && e.erros) {
				...
				// Erro de estoque negativo (BadRequest) ou erro de servidor
...
<form on:submit|preventDefault={handleSubmit}>
	<div class="border-b pb-4 mb-6">
		<h2 class="text-2xl font-bold text-gray-800">Importar Ajustes de Estoque</h2>
	...
	<div class="space-y-5">
		...
		>
			<div class="flex flex-col items-center justify-center pt-5 pb-6 text-center">
				...
				{:else}
					<p class="font-semibold text-primary-700">{fileList[0].name}</p>
		...
		<div class="text-sm text-gray-600 p-3 bg-gray-100 rounded-md border">
			...
			<ul class="list-disc list-inside ml-4 mt-1">
				<li><code class="font-bold">PRODUTO_NOME</code> (Nome exato do produto)</li>
				<li><code class="font-bold">QUANTIDADE_AJUSTE</code> (O valor a ser somado/subtraído)</li>
			</ul>
			<p class="mt-2 text-xs">
				<span class="font-bold">Exemplo:</span> Se o estoque atual de "Arroz" é <code class="font-bold">10</code> e a planilha tiver <code class="font-bold">5</code>, o novo estoque será <code class="font-bold">15</code>.
				Se a planilha tiver <code class="font-bold">-2</code>, o novo estoque será <code class="font-bold">8</code>.
			</p>
		</div>

		{#if importErrors.length > 0}
			<div
				class="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg shadow-inner max-h-48 overflow-y-auto"
				transition:fade
			>
	...
	<div class="mt-8 flex justify-between space-x-3 border-t pt-6">
		<button
			type="button"
		...
		<button
			type="submit"
			class="px-5 py-2 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition-all shadow-sm w-32 disabled:opacity-50"
			disabled={isLoading}
		>
			{#if isLoading}
