<!-- src/lib/components/CidadeForm.svelte -->
<script lang="ts">
  import { createEventDispatcher } from 'svelte';

  export let cidade: { id?: string; name: string; state: string; } = { name: '', state: '' };
  export let isEditing = false;
  
  const dispatch = createEventDispatcher();

  function handleSubmit() {
    cidade.state = cidade.state.toUpperCase();
    dispatch('save', cidade);
  }
</script>

<form on:submit|preventDefault={handleSubmit}>
  <h2 class="text-2xl font-bold mb-6">
    {isEditing ? 'Editar Cidade' : 'Adicionar Nova Cidade'}
  </h2>

  <div class="space-y-4">
    <div>
      <label for="name" class="block text-sm font-medium text-gray-700">Nome da Cidade</label>
      <input type="text" id="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" bind:value={cidade.name} required>
    </div>
    
    <div>
      <label for="state" class="block text-sm font-medium text-gray-700">Estado (UF)</label>
      <input 
        type="text" 
        id="state" 
        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" 
        placeholder="Ex: SP"
        maxlength="2"
        bind:value={cidade.state} 
        required
      >
    </div>
  </div>

  <div class="mt-8 flex justify-end space-x-3">
    <button type="button" on:click={() => dispatch('cancel')} class="bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded hover:bg-gray-300">
      Cancelar
    </button>
    <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded">
      Salvar
    </button>
  </div>
</form>
