...
<form on:submit|preventDefault={handleSubmit} class="space-y-6">
  ...
  <div class="space-y-5">
      <div>
        ...
        <input type="text" id="name" class="w-full rounded-lg border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-400 shadow-sm px-4 py-2 transition" bind:value={escola.name} required />
      </div>
      <div>
        <label for="address" class="block text-sm font-semibold text-gray-700 mb-1">Endereço (Opcional)</label>
        <input type="text" id="address" class="w-full rounded-lg border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-400 shadow-sm px-4 py-2 transition" bind:value={escola.address} />
      </div>
      <div>
        <label for="numero" class="block text-sm font-semibold text-gray-700 mb-1">N° (Opcional)</label>
        <input type="text" id="numero" class="w-full rounded-lg border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-400 shadow-sm px-4 py-2 transition" bind:value={escola.numero} />
      </div>
      <div>
        <label for="city" class="block text-sm font-semibold text-gray-700 mb-1">Cidade</label>
        <select id="city" class="w-full rounded-lg border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-400 shadow-sm px-4 py-2 transition bg-white" bind:value={escola.cityId} required>
          {#if cidades.length === 0}
            <option disabled value="">Carregando cidades...</option>
          {:else}
            {#each cidades as cidade (cidade.id)}
              <option value={cidade.id}>{cidade.name} - {cidade.state} (CEP: {formatCep(cidade.cep)}{cidade.bairro ? ` / ${cidade.bairro}` : ''})</option>
            {/each}
          {/if}
        </select>
      </div>
