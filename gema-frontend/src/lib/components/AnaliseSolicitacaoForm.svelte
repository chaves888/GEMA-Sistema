<script lang="ts">
	...
	// Filtramos itens que não têm mais um produto (produto foi excluído)
	// E já preenchemos o nome/unidade para o Svelte
	let itemsAnalisados = solicitacao.items
		.filter((item) => item.product) // Garante que o produto não seja null
		.map((item) => ({
			itemId: item.id,
	...
	// Itens que foram removidos (para aviso)
	...
	function handleSubmit() {
		if (!isNegando) {
			...
			dispatch('save', {
				...
				items: [], // Envia array vazio, pois está negando tudo
			});
		}
	}
...
<form on:submit|preventDefault={handleSalvarAnalise} class="space-y-6">
			...
			>
				...
				{#each itemsAnalisados as item, index (item.itemId)}
					<div class="grid grid-cols-5 gap-4 items-center border-b pb-4 last:border-b-0">
						...
						<div class="col-span-2">
							<label for={`approved-${index}`} class="block text-sm font-semibold text-gray-700 mb-1"
								>Aprovado</label
							>
							<input
								type="number"
	...
	<div>
		<label for="observacao" class="block text-sm font-semibold text-gray-700 mb-1"
			>Observações da Prefeitura (Obrigatório se negar)</label
		>
		<textarea
			id="observacao"
			rows="3"
