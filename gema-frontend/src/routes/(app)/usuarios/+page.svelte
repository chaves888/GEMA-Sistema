...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-4 lg:p-6 space-y-6 animate-fadeIn">
  ...
  <div class="bg-white/80 backdrop-blur-md p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row flex-wrap gap-4">
    <div class="flex-1 w-full md:min-w-[200px]">
      <label for="filterSearch" class="block text-sm font-medium text-gray-700">Buscar (Nome, Email, Escola)</label>
      <div class="relative mt-1">
        <input type="text" id="filterSearch" bind:value={filterSearch} placeholder="Digite para buscar..." class="block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm pl-10" />
    ...
    <div class="flex-1 w-full md:min-w-[150px]">
      ...
      <select id="filterProfile" bind:value={filterProfile} class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
            ...
            {#each filteredUsers as user (user.id)}
              <tr class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  {#if user.isActive}
                    <span class="px-2.5 py-0.5 inline-flex text-xs font-semibold rounded-full bg-green-100 text-green-800">Ativo</span>
                  {:else}
                    <span class="px-2.5 py-0.5 inline-flex text-xs font-semibold rounded-full bg-gray-100 text-gray-600">Inativo</span>
                  {/if}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                  <button on:click={() => openEditModal(user)} class="text-primary-600 hover:text-primary-800 font-semibold transition-colors disabled:opacity-50" disabled={isActionLoading}>Editar</button>
                  {#if user.isActive}
                    <button on:click={() => openConfirmModal(user, 'deactivate')} class="text-yellow-600 hover:text-yellow-800 font-semibold transition-colors disabled:opacity-50" disabled={isActionLoading || user.id === $session.id}>Desativar</button>
                  {:else}
                    <button on:click={() => openConfirmModal(user, 'activate')} class="text-green-600 hover:text-green-800 font-semibold transition-colors disabled:opacity-50" disabled={isActionLoading || user.id === $session.id}>Ativar</button>
                  {/if}
                  <button on:click={() => openConfirmModal(user, 'delete')} class="text-red-600 hover:text-red-800 font-semibold transition-colors disabled:opacity-50" disabled={isActionLoading || user.id === $session.id}>Excluir</button>
                </td>
              </tr>
            {/each}
          </tbody>
