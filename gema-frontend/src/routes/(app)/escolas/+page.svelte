...
    };
  ...
  async function handleSave(event: any) {
    ...
    try {
      ...
      else alert('Erro ao salvar a escola.');
      console.error(e);
    }
  }
  ...
  async function deleteEscola(escolaId: string, escolaName: string) {
    ...
    try {
      ...
      if (e && e.message) { alert(`Erro: ${e.message}`); }
      else { alert('Falha ao excluir a escola.'); }
      console.error(e);
    }
  }
...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-6 space-y-6 animate-fadeIn">
  ...
  {#if isLoading}
	...
	{:else if escolas.length === 0}
		<div class="text-center p-10 bg-white rounded-xl shadow-sm border border-dashed border-gray-300"> <p class="text-gray-600 font-semibold text-lg">Nenhuma escola encontrada.</p> <p class="text-sm text-gray-400 mt-2">Clique em “Nova Escola” para adicionar a primeira.</p> </div>
	{:else}
		<div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-md border border-gray-100 overflow-hidden">
            ...
            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Ações</th>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
								...
								<button on:click={() => deleteEscola(escola.id, escola.name)} class="text-red-600 hover:text-red-800 font-semibold transition-colors"> Excluir </button>
							</td>
						</tr>
					{/each}
				</tbody>
