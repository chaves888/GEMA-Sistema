<script lang="ts">
	...
	async function handleSaveAnalysis(event: any) {
		...
		try {
			...
			// --- CORREÇÃO NA CAPTURA DO ERRO ---
			// Agora 'e.message' vem do JSON do backend
		...
		<div>
			...
			{#if $session?.profile === 'escola'}
				<p class="text-gray-600 mt-1 text-sm">
					Crie e acompanhe os pedidos de reposição para sua escola.
				</p>
			{:else if $session?.profile === 'prefeitura'}
				<p class="text-gray-600 mt-1 text-sm">
					Analise e gerencie as solicitações recebidas das escolas.
				</p>
			{/if}
		</div>
						...
						<tr class="hover:bg-gray-50 transition-colors duration-150">
							...
							{#if $session?.profile === 'prefeitura'}
								<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
									>{s.school?.name || '(Escola Excluída)'}</td
								>
							{/if}
							...
							<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
								{#if $session?.profile === 'prefeitura' && s.status === 'pendente'}
									<button
										...
										class="px-3 py-1 rounded-full font-semibold text-xs bg-primary-100 text-primary-700 hover:bg-primary-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
									>
										Analisar
									</button>
								{/if}
								...
								{#if $session?.profile === 'escola'}
									{#if s.status === 'aprovado' || s.status === 'aprovado_parcialmente'}
										<button
											...
											disabled={isActionLoading}
											class="px-3 py-1 rounded-full font-semibold text-xs bg-green-100 text-green-700 hover:bg-green-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
										>
											Confirmar
										</button>
									{/if}
									...
									{#if s.status === 'pendente'}
										<button
											...
											disabled={isActionLoading}
											class="px-3 py-1 rounded-full font-semibold text-xs bg-red-100 text-red-700 hover:bg-red-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
										>
											Cancelar
										</button>
									{/if}
								...
								<button
									...
									disabled={isActionLoading}
									class="px-3 py-1 rounded-full font-semibold text-xs bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
								>
									Ver Detalhes
								</button>
							</td>
...
{#if solicitacaoToCancel}
	<Modal
		...
		on:close={() => {
			(showCancelModal = false), (solicitacaoToCancel = null);
		}}
		size="max-w-lg"
	>
		<div class="space-y-6">
			...
			<div>
				<label for="motivoCancelamento" class="block text-sm font-semibold text-gray-700 mb-1"
