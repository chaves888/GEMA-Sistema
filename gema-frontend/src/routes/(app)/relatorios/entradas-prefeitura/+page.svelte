<script lang="ts">
  ...
  // 1. Tipo de dado para este relatório
  ...
  // 3. Opções e inicialização do Flatpickr
  ...
  // 4. Função para gerar o relatório
  async function handleGenerateReport() {
    ...
    try {
      ...
      if (reportData?.length === 0) {
        toast.info('Nenhuma entrada encontrada no período.');
      }
    } catch (e: any) {
  ...
  // 5. Funções para PDF e Excel
  function getReportHeaders() {
    return [['Produto', 'Unid.', 'Qtd. Adicionada', 'Observação/Origem', 'Usuário']];
  }
  function getReportBody() {
    ...
    return reportData.map((row) => [
      row.productName,
      row.unit,
      row.quantidadeAdicionada,
      row.motivoObservacao || 'Ajuste Manual', // Fallback
      row.userName,
    ]);
  }
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      doc.save(`relatorio_entradas_central_${startDate}_a_${endDate}.pdf`);
    } catch (e) {
  ...
  async function handleDownloadExcel() {
    if (!reportData) return;
    try {
      const data = [...getReportHeaders(), ...getReportBody()];
      
      const worksheet = XLSX.utils.aoa_to_sheet(data);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Entradas_Central');
      
      worksheet['!cols'] = [
        { wch: 30 }, // Produto
        { wch: 10 }, // Unid.
        { wch: 20 }, // Qtd. Adicionada
        { wch: 50 }, // Observação
        { wch: 30 }  // Usuário
      ];

      XLSX.writeFile(workbook, `relatorio_entradas_central_${startDate}_a_${endDate}.xlsx`);
    } catch (e) {
...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-6 space-y-6 animate-fadeIn">
  ...
  >
    <div>
      ...
      >
        Relatório de Entradas (Estoque Central)
      </h1>
      <p class="text-gray-600 mt-1 text-sm">
        Auditoria de entradas por ajuste manual ou importação de planilha.
      </p>
    </div>
  </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            {#each reportData as row (`${row.productId}-${row.motivoObservacao}-${row.userName}`)}
              <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" title={row.motivoObservacao || ''}>
                  {row.motivoObservacao || 'Ajuste Manual'}
                </td>
