<script lang="ts">
  ...
  import { ArrowLeft, FileText, Download, Loader2, Calendar, FileSpreadsheet, Send } from 'lucide-svelte';
  ...
  let reportData: RelatorioAjusteEscolaRow[] | null = null;
  let isLoading = false;
  ...
  async function handleGenerateReport() {
    ...
    try {
      ...
      reportData = await api.post('relatorios/ajustes-perdas-escolas', dto);
  ...
  function getReportHeaders() {
    return [['Escola', 'Produto', 'Unid.', 'Motivo', 'Qtd.', 'Descrição']];
  }
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      // 2. Adicionar Logo (x, y, largura, altura)
      doc.addImage(img, 'PNG', 14, 10, 30, 15);

      // 3. Ajustar posições do texto (descer um pouco)
      doc.setFontSize(16);
      doc.text('Relatório de Ajustes e Perdas nas Escolas', 14, 35); // Y aumentado
      
      doc.setFontSize(10);
      doc.text(`Período: ${startDate} a ${endDate}`, 14, 41); // Y aumentado

      // 4. Gerar Tabela (startY ajustado)
      autoTable(doc, {
        head: getReportHeaders(),
        body: getReportBody(),
        startY: 50, // Começa mais baixo para não cobrir o título
        theme: 'grid',
        headStyles: { fillColor: [13, 71, 161] },
      });

      doc.save(`relatorio_ajustes_perdas_escolas_${startDate}_a_${endDate}.pdf`);
    } catch (e) {
  ...
  async function handleDownloadExcel() {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Ajustes_Escolas');
      ...
      XLSX.writeFile(workbook, `relatorio_ajustes_perdas_escolas_${startDate}_a_${endDate}.xlsx`);
    } catch (e) {
  ...
  async function handleConfirmEmailSend() {
    ...
    try {
      const dto = { startDate, endDate };
      const response = await api.post('relatorios/ajustes-perdas-escolas/enviar-email', dto);
  ...
  <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/70 dark:bg-gray-800/70 backdrop-blur-md p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
    <div>
      <h1 class="text-2xl lg:text-3xl font-extrabold bg-gradient-to-r from-primary-600 to-primary-400 bg-clip-text text-transparent">
        Relatório de Ajustes e Perdas (Escolas)
      </h1>
      <p class="text-gray-600 dark:text-gray-300 mt-1 text-sm">
        Auditoria de perdas, vencimentos e outros ajustes de saída manuais.
      </p>
    </div>
  </div>
    ...
    <div class="flex-1 w-full md:min-w-[150px]">
      ...
      <div class="relative mt-1">
        <input
          ...
          use:initStartDate
          class="block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm pr-10 placeholder-gray-400 dark:placeholder-gray-400"
        />
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
            {#each reportData as row (`${row.escolaId}-${row.productId}-${row.motivoCategoria}-${row.motivoObservacao}`)}
