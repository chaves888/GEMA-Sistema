<script lang="ts">
	...
	import {
		...
		BookOpen,
		PackageCheck,
		AlertCircle, // <-- 1. IMPORTAR NOVO ÍCONE
	} from 'lucide-svelte';
	...
	async function loadDashboardData() {
		...
		escolaAwaitingReceiptCount = 0;
		nutriDraftCount = 0;
		nutriPublishedCount = 0;
		escolaPublishedCardapiosCount = 0;
		stockMap.clear(); // <-- 3. LIMPAR MAPA

		try {
			dashboardData = await api.get('dashboard');

			// --- 4. LÓGICA DA COZINHEIRA ATUALIZADA ---
			if ($session?.profile === 'cozinheira') {
				// Mapeia o estoque recebido
	...
	function getStockStatusClass(status: string | undefined): string {
		...
		return 'text-gray-500'; // Bom ou N/A
	}
					...
					<div class="panel">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-4">
							{#each dashboardData.schoolStocks as schoolStock (schoolStock.schoolId)}
								<button
									type="button"
									class="school-stat-card {schoolStock.criticalItemsCount > 0
										? 'is-critical'
										: ''}"
								...
								>
									...
									<div class="school-card-body">
										<span
											class="count {schoolStock.criticalItemsCount > 0
												? 'text-red-600'
												: 'text-green-600'}"
										>
										...
										<span class="label">
											{schoolStock.criticalItemsCount === 1
												? 'Item crítico/médio'
												: 'Itens críticos/médios'}
										</span>
									</div>
								</button>
							{/each}
						...
						{#if dashboardData.recentSolicitacoes?.length > 0}
							<ul class="space-y-3">
								{#each dashboardData.recentSolicitacoes as s}
									<li
										class="flex justify-between text-sm border-b border-gray-100 pb-2 last:border-b-0"
									>
										<span class="text-gray-500"
											>{format(parseISO(s.createdAt), 'dd/MM/yy HH:mm')}</span
										>
										<span
											class="px-2 py-0.5 rounded-full text-xs font-semibold {getStatusClass(
												s.status
											)}">{formatStatus(s.status)}</span
										>
									</li>
								{/each}
							</ul>
										...
										{#each refeicaoManhaHoje.products as item (item.id)}
											...
											<li class="ingredient-item">
												...
												{#if stockInfo && (stockInfo.status === 'Crítico' || stockInfo.status === 'Médio')}
													<span
														class="stock-warning {getStockStatusClass(stockInfo.status)}"
														title="Estoque: {stockInfo.quantity} / {stockInfo.minStock}"
													>
...
<style>
	...
	/* --- NOVOS ESTILOS PARA COZINHEIRA --- */
	.meal-card {
		background: linear-gradient(to bottom, white, #f9fafb); /* gray-50 */
...
.meal-header.afternoon {
	...
	.ingredient-title {
		...
		display: block; /* Garante que o 'mt-4' funcione */
		margin-top: 1rem;
	}
	.ingredient-list {
	...
	.ingredient-item { /* Esta classe foi adicionada no HTML */
		display: flex;
