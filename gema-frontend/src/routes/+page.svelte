...
  // Controla qual formulário exibir: 'login' ou 'forgot'
  let view: 'login' | 'forgot' = 'login';
...
      if (response.ok) {
        successMessage = data.message || 'Solicitação enviada com sucesso!';
      } else {
        errorMessage = data.message || 'Ocorreu um erro.';
      }
    } catch (error) {
      console.error('Erro de conexão:', error);
...
  function switchTo(newView: 'login' | 'forgot') {
    view = newView;
    email = '';
    password = '';
    errorMessage = null;
    ...
    {#if view === 'login'}
      <form class="space-y-4 mt-6" on:submit|preventDefault={handleLogin}>
        ...
        <div class="text-right">
          <button
            type="button"
            on:click={() => switchTo('forgot')}
            class="text-sm font-medium text-accent-300 hover:text-accent-200 transition"
          >
            Esqueci minha senha
          </button>
        </div>
