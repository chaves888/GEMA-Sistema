...
    const awaitingReceiptCount = await this.solicitacaoRepository.count({
      where: {
        school: { id: schoolId },
        status: In([
          SolicitacaoStatus.APROVADO,
          SolicitacaoStatus.APROVADO_PARCIALMENTE,
        ]),
      },
    });
...
        publishedCardapios: publishedCardapiosCount,
...
  // --- INÍCIO DA MODIFICAÇÃO (COZINHEIRA) ---
  private async getCozinheiraData(user: User) {
    if (!user.school?.id) return { error: 'Usuário não associado a uma escola.' };
    
  	// 1. Pega o cardápio de hoje
  	const today = format(new Date(), 'yyyy-MM-dd');
    const currentCardapio = await this.cardapioRepository.findOne({
      where: { 
...
  	// 2. Pega o estoque da escola
  	const escolaEstoque = await this.estoqueService.getEscolaEstoque(user.school.id);

  	// 3. Retorna ambos
  	return { currentCardapio, escolaEstoque };
  }
  // --- FIM DA MODIFICAÇÃO ---
}