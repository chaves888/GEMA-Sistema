import {
	Injectable, NotFoundException, UnauthorizedException, BadRequestException, ConflictException, ForbiddenException
} from '@nestjs/common';
...
import {
	...
	MotivoMovimentacao,
	TipoMovimentacao,
} from 'src/estoque/entities/estoque-movimentacao.entity';
// --- FIM NOVOS IMPORTS ---
...
export class SolicitacoesService {
	...
	async analyze(
		id: string,
		analyzeDto: AnalyzeSolicitacaoDto,
		user: User, // <-- Recebe o usuário da prefeitura
	): Promise<Solicitacao> {
		const queryRunner = this.dataSource.createQueryRunner();
		await queryRunner.connect();
		await queryRunner.startTransaction();
		...
		try {
			...
			if (solicitacao.status !== SolicitacaoStatus.PENDENTE) {
				throw new BadRequestException(
					`Esta solicitação não está mais pendente (status atual: ${solicitacao.status}).`,
				);
			}
			...
			if (!analyzeDto.items || analyzeDto.items.length === 0) {
				throw new BadRequestException(
					'Para aprovar (total ou parcialmente), é necessário fornecer a análise dos itens.',
				);
			}

			const hasZeroApprovedItem = analyzeDto.items.some(
				(item) => (item.quantityApproved ?? 0) === 0,
			);
			if (hasZeroApprovedItem && !analyzeDto.observacaoPrefeitura) {
				throw new BadRequestException(
					'A observação é obrigatória se algum item individual for negado (quantidade aprovada 0).',
				);
			}

			// Validação prévia de estoque (loop rápido)
			for (const itemDto of analyzeDto.items!) {
				const itemSolicitado = solicitacao.items.find((item) => item.id === itemDto.itemId);
				if (!itemSolicitado || !itemSolicitado.product) {
					continue; // Ignora itens com produtos excluídos
				}
				
				const quantityApproved = itemDto.quantityApproved ?? 0;
				if (quantityApproved < 0)
					throw new BadRequestException(
						`Quantidade aprovada não pode ser negativa para o item ${itemSolicitado.product.name}.`,
					);

				if (quantityApproved > itemSolicitado.quantityRequested) {
					throw new BadRequestException(
						`Quantidade aprovada (${quantityApproved}) para "${itemSolicitado.product.name}" não pode ser maior que a solicitada (${itemSolicitado.quantityRequested}).`,
					);
				}
				...
				if (quantityApproved > 0) {
					...
					if (!estoquePrefeitura || estoquePrefeitura.quantity < quantityApproved) {
						throw new ConflictException(
							`Estoque insuficiente na prefeitura para "${itemSolicitado.product.name}". Necessário: ${quantityApproved}, Disponível: ${estoquePrefeitura?.quantity ?? 0}`,
						);
					}
				}
			}

			// Loop principal de salvamento (agora com log)
			const itemsParaSalvar: SolicitacaoItem[] = [];
			for (const itemDto of analyzeDto.items!) {
				const itemSolicitado = solicitacao.items.find((item) => item.id === itemDto.itemId);
				if (!itemSolicitado || !itemSolicitado.product) { // Ignora produtos excluídos
					continue;
				}
				...
				if (quantityApproved > 0) {
					// 1. Atualiza o estoque da prefeitura
					const estoquePrefeitura = await queryRunner.manager.findOneOrFail(
						PrefeituraEstoque,
						{
							where: { product: { id: itemSolicitado.product.id } },
							lock: { mode: 'pessimistic_write' },
					...
					if (estoquePrefeitura.quantity < quantityApproved) {
						throw new ConflictException(
							`Concorrência detectada: Estoque insuficiente na prefeitura para "${itemSolicitado.product.name}" após lock.`,
						);
					}
					estoquePrefeitura.quantity -= quantityApproved;
					await queryRunner.manager.save(PrefeituraEstoque, estoquePrefeitura);

					// --- 2. ADICIONA O LOG DE MOVIMENTAÇÃO (SAÍDA) ---
					const movimentacao = queryRunner.manager.create(EstoqueMovimentacao, {
						contexto: 'prefeitura',
						product: itemSolicitado.product,
						school: null,
						user: user, // O usuário da prefeitura que aprovou
						tipo: TipoMovimentacao.SAIDA,
						quantidade: quantityApproved,
						motivoCategoria: MotivoMovimentacao.SAIDA_SOLICITACAO,
						motivoObservacao: `Saída p/ Solicitação Escola: ${solicitacao.school.name} (ID: ${solicitacao.id.split('-')[0]})`,
					});
	...
	async confirmRecebimento(
		id: string,
		confirmDto: ConfirmRecebimentoDto,
		user: User, // <-- Recebe o usuário da escola
	): Promise<Solicitacao> {
		...
		try {
			...
			if (solicitacao.school.id !== user.school?.id) {
				throw new ForbiddenException(
					'Você não tem permissão para confirmar o recebimento desta solicitação.',
				);
			}
			if (
				...
				throw new BadRequestException(
					`Esta solicitação não pode ser confirmada (status atual: ${solicitacao.status}).`,
				);
			}

			const itemsParaSalvarSolicitacao: SolicitacaoItem[] = [];
			const estoquesParaSalvarEscola: EscolaEstoque[] = [];
			const movimentacoesParaSalvar: EstoqueMovimentacao[] = []; // <-- Array de Logs
			...
			for (const itemDto of items) {
				...
				if (!itemSolicitado || !itemSolicitado.product) {
					console.warn(
						`Item com ID ${itemDto.itemId} não encontrado ou produto removido na solicitação ${id} durante a confirmação.`,
					);
					continue;
				}
				...
				if (quantityReceived < 0) {
					throw new BadRequestException(
						`A quantidade recebida não pode ser negativa para "${itemSolicitado.product.name}".`,
					);
				}
				if (quantityReceived > maxRecebivel) {
					throw new BadRequestException(
						`A quantidade recebida (${quantityReceived}) para "${itemSolicitado.product.name}" não pode ser maior que a aprovada (${maxRecebivel}).`,
					);
				}
				...
				if (quantityReceived > 0) {
					...
					// --- 2. ADICIONA O LOG DE MOVIMENTAÇÃO (ENTRADA) ---
					const movimentacao = queryRunner.manager.create(EstoqueMovimentacao, {
						contexto: 'escola',
						product: itemSolicitado.product,
						school: solicitacao.school,
						user: user, // O usuário da escola que confirmou
						tipo: TipoMovimentacao.ENTRADA,
						quantidade: quantityReceived,
						motivoCategoria: MotivoMovimentacao.ENTRADA_SOLICITACAO,
						motivoObservacao: `Recebimento da Solicitação ID: ${solicitacao.id.split('-')[0]}`,
					});
					movimentacoesParaSalvar.push(movimentacao);
					// --- FIM DO LOG ---
				}
			} // Fim do loop 'for'
			...
			if (finalDivergence && !observacaoEscola) {
				throw new BadRequestException(
					'A observação é obrigatória quando há divergência (seja na quantidade ou marcada manualmente).',
				);
			}
			...
			await queryRunner.manager.save(EstoqueMovimentacao, movimentacoesParaSalvar); // <-- Salva os logs
			await queryRunner.manager.save(Solicitacao, solicitacao);
	...
	async cancelar(id: string, user: User, cancelDto: CancelSolicitacaoDto): Promise<Solicitacao> {
		...
		if (solicitacao.status !== SolicitacaoStatus.PENDENTE) {
			throw new BadRequestException(
				`Apenas solicitações pendentes podem ser canceladas (status atual: ${solicitacao.status}).`,
			);
		}
